...
- 📊 **複数行ヘッダー**: `colSpan`/`rowSpan` による柔軟なヘッダーレイアウト
...
Excelでは `01` や `007` のような値は数値として扱われ、先頭のゼロが消えてしまいます。
文字列として保持するには、以下の方法があります。

**方法1: データを文字列として渡す（推奨）**

```typescript
rows: [
  { code: '007' },      // ✅ 文字列として渡す → "007" のまま保持
  { code: '0123' },     // ✅ "0123" のまま保持
  { zipCode: '00456' }  // ✅ "00456" のまま保持
]
```
...
| `'007'` | 文字列 | 007 ✅ |
| `7` | 数値 | 7 |
| `'01234567890'` | 文字列 | 01234567890 ✅ |
| `1234567890` | 数値 | 1234567890 |

> 電話番号、郵便番号、社員コードなど先頭にゼロが含まれる可能性があるデータは、必ず文字列として渡してください。
