# xlkit

<p align="center">
  <img src="./logo.png" alt="xlkit Logo" width="200" />
</p>

[ExcelJS](https://github.com/exceljs/exceljs) のための宣言的スキーマベースラッパーです。
シンプルなスキーマでExcelの構造を定義するだけで、スタイル、フォーマット、レイアウトをxlkitが自動で処理します。

[English README](./README_en.md)

## 目次

- [特徴](#特徴)
- [インストール](#インストール)
- [クイックスタート](#クイックスタート)
- [クイックリファレンス](#クイックリファレンス)
  - [スタイルプロパティ一覧](#スタイルプロパティ一覧)
  - [罫線プリセット](#罫線プリセット)
  - [スタイル優先順位](#スタイル優先順位)
- [APIリファレンス](#apiリファレンス)
- [よくある使用パターン](#よくある使用パターン)
- [完全な例](#完全な例)

## 特徴

- 📝 **宣言的スキーマ**: データとスキーマを一箇所で定義
- 🎨 **柔軟なスタイル設定**: タイトル、ヘッダー、行、列、セルの7段階でスタイルを適用可能
- 🔗 **自動結合**: 同じ値を持つ縦方向のセルを自動的に結合
- 📏 **自動列幅**: コンテンツ（全角文字を含む）に基づいて列幅をスマートに計算
- 🌈 **Hexカラー**: 標準的な6桁のHexコード（`#FF0000`）を直接使用可能
- 🌐 **ユニバーサル**: Node.js（ファイル出力）とブラウザ（`Uint8Array` 出力）の両方で動作

## インストール

```bash
npm install xlkit
```

## クイックスタート

```typescript
import { createWorkbook } from 'xlkit';

await createWorkbook().addSheet({
  name: 'Users',
  headers: [
    { key: 'id', label: 'ID', width: 10 },
    { key: 'name', label: '氏名', width: 20 },
    { key: 'role', label: '役割', width: 'auto', merge: 'vertical' },
    {
      key: 'isActive',
      label: 'ステータス',
      format: (val) => val ? '有効' : '無効',
      style: (val) => ({ font: { color: val ? '#00AA00' : '#FF0000' } })
    }
  ],
  rows: [
    { id: 1, name: 'Alice', role: 'Admin', isActive: true },
    { id: 2, name: 'Bob', role: 'User', isActive: true },
    { id: 3, name: 'Charlie', role: 'User', isActive: false }
  ],
  borders: 'all'
}).save('users.xlsx');
```

## クイックリファレンス

### スタイルプロパティ一覧

#### フォント (`font`)

| プロパティ | 型 | 取りうる値 | デフォルト | 説明 |
|-----------|-----|-----------|----------|------|
| `name` | `string` | `'Arial'`, `'メイリオ'`, etc. | システム依存 | フォント名 |
| `size` | `number` | `8` - `72` | `11` | フォントサイズ |
| `bold` | `boolean` | `true` / `false` | `false` | 太字 |
| `italic` | `boolean` | `true` / `false` | `false` | 斜体 |
| `underline` | `boolean` | `true` / `false` | `false` | 下線 |
| `strike` | `boolean` | `true` / `false` | `false` | 取り消し線 |
| `color` | `string` | `'#RRGGBB'` 形式 | `'#000000'` | 文字色 |

```typescript
// 使用例
{ font: { name: 'Arial', size: 12, bold: true, color: '#FF0000' } }
```

#### 塗りつぶし (`fill`)

| プロパティ | 型 | 取りうる値 | デフォルト | 説明 |
|-----------|-----|-----------|----------|------|
| `color` | `string` | `'#RRGGBB'` 形式 | なし | 背景色 |

```typescript
// 使用例
{ fill: { color: '#FFFF00' } }  // 黄色の背景
```

**よく使う色:**

| 色 | Hexコード | 用途例 |
|----|----------|--------|
| 赤 | `#FF0000` | エラー、警告 |
| 緑 | `#00AA00` | 成功、有効 |
| 青 | `#4472C4` | ヘッダー、リンク |
| 黄 | `#FFFF00` | ハイライト |
| グレー | `#F2F2F2` | 交互行の背景 |
| 薄い青 | `#DEEBF7` | 選択行 |
| 薄い緑 | `#C6EFCE` | 正の値 |
| 薄い赤 | `#FFC7CE` | 負の値 |

#### 配置 (`alignment`)

| プロパティ | 型 | 取りうる値 | デフォルト | 説明 |
|-----------|-----|-----------|----------|------|
| `horizontal` | `string` | `'left'`, `'center'`, `'right'` | `'left'` | 水平方向の配置 |
| `vertical` | `string` | `'top'`, `'middle'`, `'bottom'` | `'bottom'` | 垂直方向の配置 |
| `wrapText` | `boolean` | `true` / `false` | `false` | テキストの折り返し |

```typescript
// 使用例
{ alignment: { horizontal: 'center', vertical: 'middle', wrapText: true } }
```

### 罫線プリセット

| 値 | 説明 | 見た目 |
|----|------|--------|
| `'none'` | 罫線なし（デフォルト） | セル境界が見えない |
| `'all'` | すべてのセルに格子状の罫線 | 表形式 |
| `'outer'` | データ領域の外枠のみ | 外側だけ囲む |
| `'header-body'` | ヘッダー行の下に太めの線 | ヘッダーを強調 |

### 列幅オプション

| 値 | 型 | 説明 |
|----|-----|------|
| `10`, `20` など | `number` | 固定幅（文字数） |
| `'auto'` | `string` | コンテンツに基づいて自動計算 |
| 未指定 | - | `autoWidth` 設定に従う、なければデフォルト幅 |

### スタイル優先順位

スタイルは以下の順序で適用され、後のスタイルが前のスタイルを上書きします。

**ヘッダー行:**
```
styles.all → styles.header → headers[].label.style
```

**データ行:**
```
styles.all → styles.body → styles.column[key] → styles.row() → headers[].style → rows[].{key}.style
```

```
優先度: 低 ──────────────────────────────────────────────────────────→ 高

┌──────────┐   ┌──────────┐   ┌────────────────┐   ┌───────────┐   ┌──────────────┐   ┌─────────────────┐
│styles.all│ → │styles.body│ → │styles.column[key]│ → │styles.row()│ → │headers[].style│ → │rows[].{key}.style│
└──────────┘   └──────────┘   └────────────────┘   └───────────┘   └──────────────┘   └─────────────────┘
   全体          ボディ全体       列全体             行全体          列定義のスタイル    セル個別のスタイル
```

## APIリファレンス

### 基本構造

```typescript
createWorkbook().addSheet({
  name: string,              // シート名（必須）
  headers: HeaderDef[],      // 列定義（必須）
  rows: RowData[],           // データ行（必須）
  title?: TitleConfig,       // タイトル行
  styles?: StylesConfig,     // 全体スタイル設定
  borders?: BorderPreset,    // 罫線プリセット
  autoWidth?: AutoWidthConfig // 自動列幅設定
})
```

### ヘッダー定義 (`HeaderDef`)

| プロパティ | 型 | 必須 | デフォルト | 説明 |
|-----------|-----|:----:|----------|------|
| `key` | `string` | ✅ | - | データのプロパティキー |
| `label` | `string \| LabelConfig` | ✅ | - | ヘッダーテキスト |
| `width` | `number \| 'auto'` | - | 未指定 | 列幅 |
| `merge` | `'vertical'` | - | 未指定 | 縦方向の自動結合 |
| `format` | `string \| FormatFn` | - | 未指定 | 表示フォーマット |
| `style` | `XLStyle \| StyleFn` | - | 未指定 | 列のスタイル |

```typescript
headers: [
  // 基本的な定義
  { key: 'id', label: 'ID', width: 10 },

  // 自動結合
  { key: 'dept', label: '部署', merge: 'vertical' },

  // フォーマット指定（Excel形式）
  { key: 'salary', label: '給与', format: '¥#,##0' },

  // フォーマット指定（関数）
  { key: 'rate', label: '割合', format: (val) => `${val}%` },

  // 固定スタイル
  { key: 'total', label: '合計', style: { font: { bold: true } } },

  // 条件付きスタイル（関数）
  {
    key: 'status',
    label: 'ステータス',
    style: (val, row, index) => val === 'OK'
      ? { font: { color: '#00AA00' } }
      : { font: { color: '#FF0000' } }
  },

  // ヘッダーセル自体にスタイルを適用
  {
    key: 'name',
    label: { value: '名前', style: { font: { bold: true, color: '#FFFFFF' }, fill: { color: '#4472C4' } } }
  }
]
```

### フォーマット文字列

| 用途 | フォーマット | 表示例 |
|------|------------|--------|
| 通貨（ドル） | `'$#,##0'` | $1,000 |
| 通貨（円） | `'¥#,##0'` | ¥1,000 |
| 小数点2桁 | `'#,##0.00'` | 1,000.00 |
| パーセント | `'0.00%'` | 12.34% |
| 日付 | `'yyyy-mm-dd'` | 2025-01-15 |
| 日付（日本式） | `'yyyy/mm/dd'` | 2025/01/15 |

### データ行 (`rows`)

```typescript
rows: [
  // シンプルな値
  { id: 1, name: 'Alice', salary: 50000 },

  // セルにスタイルを適用
  {
    id: 2,
    name: { value: 'Bob', style: { font: { bold: true } } },
    salary: 60000
  }
]
```

### タイトル行 (`title`)

| プロパティ | 型 | 説明 |
|-----------|-----|------|
| `label` | `string \| string[]` | タイトルテキスト（配列で複数行） |
| `style` | `XLStyle` | タイトルのスタイル |

```typescript
// 単一行タイトル
title: {
  label: '従業員リスト 2025',
  style: {
    fill: { color: '#4472C4' },
    font: { color: '#FFFFFF', bold: true, size: 14 },
    alignment: { horizontal: 'center' }
  }
}

// 複数行タイトル
title: {
  label: ['株式会社サンプル', '月次報告書 2025年1月'],
  style: { font: { bold: true }, alignment: { horizontal: 'center' } }
}
```

### 全体スタイル設定 (`styles`)

| プロパティ | 型 | 説明 |
|-----------|-----|------|
| `all` | `XLStyle` | 全セルに適用 |
| `header` | `XLStyle` | ヘッダー行に適用 |
| `body` | `XLStyle` | データ行全体に適用 |
| `row` | `(data, index) => XLStyle` | 行ごとに動的スタイル |
| `column` | `{ [key]: XLStyle }` | 列ごとのスタイル |

```typescript
styles: {
  all: { font: { name: 'メイリオ', size: 10 } },
  header: { fill: { color: '#4472C4' }, font: { color: '#FFFFFF', bold: true } },
  body: { alignment: { vertical: 'middle' } },
  row: (data, index) => index % 2 === 1 ? { fill: { color: '#F2F2F2' } } : {},
  column: {
    price: { alignment: { horizontal: 'right' } },
    name: { font: { bold: true } }
  }
}
```

### 列幅自動調整 (`autoWidth`)

```typescript
// シンプルに有効化
autoWidth: true

// 詳細設定
autoWidth: {
  enabled: true,           // 有効/無効
  padding: 2,              // 余白（文字数）
  headerIncluded: true,    // ヘッダーも計算に含む
  charWidthConstant: 1.2   // 幅計算の係数
}
```

### 出力メソッド

| メソッド | 環境 | 説明 |
|---------|------|------|
| `save(filename, options?)` | Node.js | ファイルに保存 |
| `download(filename, options?)` | ブラウザ | ダウンロードを開始 |
| `saveToBuffer(options?)` | 共通 | `Uint8Array` を取得 |

```typescript
// オプション
interface SaveOptions {
  timeout?: number;  // タイムアウト（ミリ秒）、デフォルト: 10000
}

// 使用例
await workbook.save('output.xlsx');
await workbook.save('output.xlsx', { timeout: 30000 });
await workbook.download('output.xlsx');
const buffer = await workbook.saveToBuffer();
```

## よくある使用パターン

### 交互行の背景色

```typescript
styles: {
  row: (_, index) => index % 2 === 1 ? { fill: { color: '#F2F2F2' } } : {}
}
```

### 条件付き書式（値に応じた色分け）

```typescript
headers: [
  {
    key: 'score',
    label: '点数',
    style: (val) => {
      if (val >= 80) return { font: { color: '#006100' }, fill: { color: '#C6EFCE' } };
      if (val >= 60) return { font: { color: '#9C5700' }, fill: { color: '#FFEB9C' } };
      return { font: { color: '#9C0006' }, fill: { color: '#FFC7CE' } };
    }
  }
]
```

### 部署ごとにセルを結合

```typescript
headers: [
  { key: 'dept', label: '部署', merge: 'vertical', style: { alignment: { vertical: 'middle' } } },
  { key: 'name', label: '名前' },
  { key: 'role', label: '役職' }
]
// rows は部署でソートしておく
```

### 通貨フォーマットと右寄せ

```typescript
headers: [
  {
    key: 'price',
    label: '価格',
    format: '¥#,##0',
    style: { alignment: { horizontal: 'right' } }
  }
]
```

### ヘッダーを目立たせる

```typescript
styles: {
  header: {
    fill: { color: '#4472C4' },
    font: { color: '#FFFFFF', bold: true, size: 12 },
    alignment: { horizontal: 'center' }
  }
}
```

## 完全な例

```typescript
import { createWorkbook } from 'xlkit';

await createWorkbook().addSheet({
  name: 'Employees',
  title: {
    label: '従業員リスト 2025',
    style: {
      fill: { color: '#1F4E79' },
      font: { color: '#FFFFFF', bold: true, size: 16 },
      alignment: { horizontal: 'center' }
    }
  },
  headers: [
    {
      key: 'dept',
      label: '部署',
      merge: 'vertical',
      style: { alignment: { vertical: 'middle', horizontal: 'center' } }
    },
    { key: 'name', label: '名前', width: 20 },
    {
      key: 'salary',
      label: '給与',
      format: '¥#,##0',
      style: (val) => val > 100000
        ? { font: { color: '#006100', bold: true }, fill: { color: '#C6EFCE' } }
        : {}
    },
    {
      key: 'status',
      label: 'ステータス',
      format: (val) => val ? '在籍' : '退職',
      style: (val) => ({ font: { color: val ? '#006100' : '#9C0006' } })
    }
  ],
  rows: [
    { dept: 'Engineering', name: 'Alice', salary: 120000, status: true },
    { dept: 'Engineering', name: 'Bob', salary: 80000, status: true },
    { dept: 'Sales', name: 'Charlie', salary: 95000, status: true },
    { dept: 'Sales', name: 'Diana', salary: 88000, status: false }
  ],
  styles: {
    all: { font: { name: 'メイリオ', size: 10 } },
    header: { fill: { color: '#4472C4' }, font: { color: '#FFFFFF', bold: true } },
    row: (_, index) => index % 2 === 1 ? { fill: { color: '#F2F2F2' } } : {}
  },
  borders: 'all',
  autoWidth: true
}).save('employees.xlsx');
```

## ライセンス

MIT
